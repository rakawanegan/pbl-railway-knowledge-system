...
![image.png](./assets/Resultmap.png)
...
次に間違えていた部分について考察します。全てのLLMが間違えていたような問題には前後に似たような文章が含まれていました。例えば、２章の穴埋問題では全てのLLMが不正解となっていました。この問題の答えとなる部分には、「列車等の運転に直接関係する作業を行う係員」と「施設及び車両の保守その他これに類する作業を行う係員」という類似する２つの係員の基準が並んで記載されており、検索においてノイズになった可能性が考えられます。  
また、問題の種類と正答率には関係が見られないという意外な結果となりました。

また、各LLMの答えの差異については、GPTs(No Knowledge)では具体的な数字についてのハルシネーションが頻発しており、他のLLMではハルシネーションが少ない印象でした。  
また、GPTs(Markdown Knowledge)とRailwayKnowledgeSystem(BERT) + GPTs(No Knowledge)を比較するとGPTs(Markdown Knowledge)では正解できていた数例に対してRailwayKnowledgeSystem(BERT) + GPTs(No Knowledge)ではコンテキストに該当の箇所含まれていないことが原因でわからないと回答しているものが見られました。

最後に応用性について、本製品はMarkdown形式のKnowledgeさえ集めることができれば他のシステムに容易に応用することができます。応用例を挙げるとするならば就活ナレッジシステムや院試勉強ナレッジシステム、オープンキャンパスナレッジシステムなどが考えられます。
